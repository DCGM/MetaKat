# Trainer, predicter and evaluater for LayoutLMv3 fine-tuning complete with three modals
...
class MultiLabelNERTrainer(Trainer):
    ...
    def __init__(self, *args, class_weights=None, **kwargs):
        ...
        self.class_weights = class_weights  # Store for use later
        self.loss_fct = None  # Delay creation until we have access to the model/device
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Lazily initialize loss function with weights on the correct device
        if self.loss_fct is None:
            if self.class_weights is not None:
                self.loss_fct = BCEWithLogitsLoss(weight=self.class_weights.to(model.device))
            else:
...
def prepare_dataset(cache_dir, split):
    ...
    def prepare(ex):
        enc = processor(
            ex['image'], ex['tokens'], boxes=ex['bboxes'],
...
def evaluate(args):
    ...
    report = classification_report(
        ...
        target_names=[config.id2label[i] for i in range(33)],
        zero_division=0
    )
    print(report)
...
def predict(args):
    ...
    def prepare_examples(examples):
        ...
        enc["slova"] = examples['tokens']
        enc["word_ids"] = [
            [w if w is not None else -1 for w in encoding.word_ids]
